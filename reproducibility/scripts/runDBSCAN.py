...
def runSingle(runProgram, options, ifile, procs) :
  ...
  #print(out)
  # if (procs == 0):
  #   myProgOutput += ">numprocs*" + `procs+1` + "*\n"
  #   #print(">numprocs*" + `procs+1` + "*")
  # else:
  #   myProgOutput += ">numprocs*" + `procs` + "*\n"
  #   #print(">numprocs*" + `procs` + "*")
  if "core clustering has error" in out:
    raise NameError("clustering error detected")
...
def runTest(runProgram, checkProgram, dataDir, test, rounds, procs, noOutput) :
    ...
    times, singleOut = runSingle(runProgram, runOptions, longInputNames, procs)
    myOut += singleOut
    if (noOutput == 0) :
    ...
    #       + ptimes)
    print(myOut)
    return [weight,times,myOut]
    
def averageTime(times) :
    return sum(times)/len(times)
    

def timeAll(name, runProgram, checkProgram, dataDir, tests, rounds, procs, message, noOutput,
  ...
  try:
    ...
    allTestOutputs = ""
    for (weight,times,testOut) in results:
      allTestOutputs += testOut
    myTextFile = open("../" + message + "-" + timestr + ".txt", "a")
...
def getArgs() :
  ...
  addToDatabase = False;#getOption("-d")
  processors = int(getArg("-p", 0))
  rounds = int(getArg("-r", 1))
  message = getArg("-m", "")
  return (noOutput, rounds, addToDatabase, processors, message)

def timeAllArgs(runProgram, problem, checkProgram, dataDir, tests) :
    (noOutput, rounds, addToDatabase, procs, message) = getArgs()
    name = os.path.basename(os.getcwd())
    timeAll(name, runProgram, checkProgram, dataDir, tests, rounds, procs, message, noOutput, addToDatabase, problem)
